...
exec_detection = ['load_image'] # options are 'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'; options not in the list will be loaded from file
...
while True:
    try:
        ...
        if 'perform_tracking' in exec_list:
            ...
            association.associate_and_update(manager, meas_list_lidar, KF, lidar)
            
            # associate all camera measurements to all tracks
            # association.associate_and_update(manager, meas_list_cam, KF, camera)
