...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for detection in detections:
                ...
                center_dists =[x - box.center_x, y - box.center_y, z - box.center_z]
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                label_shape = Polygon(label_corners)
                detect_shape = Polygon(detect_corners)
                iou = label_shape.intersection(detect_shape).area/label_shape.union(detect_shape).area
                ## step 6 : if IOU exceeds min_iou threshold, store [iou,dist_x, dist_y, dist_z] in matches_lab_det and increase the TP count
                if iou > min_iou:
                    matches_lab_det.append([iou, *center_dists])
                    true_positives += 1
            #######
            ####### ID_S4_EX1 END #######     
    ...
    false_negatives = np.sum(labels_valid.astype(int)) - true_positives
...
def compute_performance_stats(det_performance_all):
    ...
    POS, TP, FN, FP = 0, 0, 0, 0
    
    for (pos, tp, fn, fp) in pos_negs:
        POS += pos; TP += tp; FN += fn; FP += fp
